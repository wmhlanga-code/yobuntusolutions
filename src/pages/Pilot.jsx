...
export default function Pilot() {
  return (
    <div>
      <SEO
        title="Pilot — Divine Project | Proving the Growth Model"
        description="Divine Project is YoBuntu Solutions' first pilot: a restaurant and culinary business in South Africa. We are building a full support ecosystem around it and documenting every result to prove the model works."
        path="/pilot"
      />

      {/* Hero */}
      <section style={{ position: 'relative', overflow: 'hidden', minHeight: 460, display: 'flex', alignItems: 'center' }}>
        <img src={images.plating} alt="" aria-hidden="true" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(100deg, rgba(8,80,65,0.94) 0%, rgba(8,80,65,0.80) 55%, rgba(8,80,65,0.45) 100%)' }} />
        <div className="section-wrap" style={{ position: 'relative', color: '#fff', width: '100%', paddingTop: '4.5rem', paddingBottom: '4.5rem' }}>
          ...
          <h1 style={{ fontSize: 'clamp(1.9rem, 4.5vw, 3rem)', maxWidth: 720, marginBottom: '1rem', color: '#fff' }}>
            Divine Project: proving the model in the real world
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.9)', fontWeight: 300, maxWidth: 620, lineHeight: 1.8, fontSize: '1.05rem' }}>
            Rather than convince investors with theory, we're proving the YoBuntu model on a real business. Divine Project is a restaurant and culinary business in South Africa — and our job is to build an entire support ecosystem around it.
          </p>
        </div>
      </section>
          ...
          <div className="split">
            <div>
              ...
              <p style={{ color: 'var(--text-muted)', lineHeight: 1.8, marginBottom: '1rem' }}>
                The goal isn't only to help Divine Project sell more meals. It's to surround the business with the relationships and tools that make sustained growth possible — the same thing we'd do for any partner.
              </p>
              <p style={{ color: 'var(--text-muted)', lineHeight: 1.8 }}>
                As Divine Project grows, we document everything. Those results become the proof that the YoBuntu model works — the foundation for the next case study, and the one after that.
              </p>
            </div>
            <div className="split-media">
              <img className="media" src={images.kitchen} alt="Chef preparing food in a professional kitchen" style={{ height: 360 }} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: 'var(--bg-secondary)', borderTop: '1px solid var(--border)' }}>
        <div className="section-wrap">
          ...
          <h2 style={{ fontSize: '1.9rem', marginBottom: '2.5rem' }}>Connections that turn a restaurant into a business</h2>
